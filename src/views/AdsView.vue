    ...
    <section class="ads-grid">
      <article class="ads-card form-card">
        ...
        <div class="form-grid">
          <label>
            <span>Title</span>
            <input v-model.trim="form.title" type="text" maxlength="160" placeholder="A sharp campaign headline" />
          </label>
          <label>
            <span>Subtitle</span>
            <input v-model.trim="form.subtitle" type="text" maxlength="200" placeholder="Optional secondary line" />
          </label>
          <label class="full">
            <span>Body</span>
            <textarea v-model.trim="form.body" rows="5" maxlength="1800" placeholder="Describe the offer, place, or event." />
          </label>
          <label>
            <span>Placement</span>
          ...
          <label>
            <span>CTA text</span>
            <input v-model.trim="form.cta_text" type="text" maxlength="80" placeholder="Book now / Learn more" />
          </label>
          <label class="full">
      ...
      <article class="ads-card preview-card">
        ...
        <div class="preview-modal">
          <div class="preview-media" :class="{ empty: !form.image_url }">
            <img v-if="form.image_url" :src="form.image_url" alt="Campaign preview" />
            <div v-else class="preview-empty">Your creative will appear here</div>
          </div>
          <div class="preview-copy">
            <span class="preview-kicker">Sponsored</span>
            <h3>{{ form.title || 'Campaign title' }}</h3>
            <p class="preview-subtitle">{{ form.subtitle || 'Optional supporting message for the popup.' }}</p>
            <p class="preview-body">{{ form.body || 'Write a concise but persuasive body. This copy will be shown inside the popup on Map or Community.' }}</p>
            <div class="preview-tags">
              <span>{{ auth.roleLabel }}</span>
              <span>{{ form.placement === 'map' ? 'Map placement' : 'Community placement' }}</span>
              <span v-if="selectedLinkedPost">Opens linked post</span>
            </div>
            <button class="preview-cta" type="button">{{ form.cta_text || 'Learn more' }}</button>
          </div>
        </div>
      </article>
    </section>
